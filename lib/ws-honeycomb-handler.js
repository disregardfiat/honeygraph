...
export class WSHoneycombHandler extends EventEmitter {
  ...
  handleCheckpoint(ws, msg) {
    ...
    // Checkpoint should include all operations up to this block
    // Operations are tracked per 100 blocks, so estimate the expected index
    const estimatedOperations = 100 * 20; // Rough estimate: ~20 operations per block
    if (lastOperationIndex < estimatedOperations) {
      console.log(`[Honeygraph] Requesting missing operations from ${lastOperationIndex + 1} for checkpoint at block ${blockNum}`);
      this.protocolAdapter.sendResponse(ws, 'request_missing', {
        from: lastOperationIndex + 1,
        to: lastOperationIndex + 1000 // Request in chunks
      });
    }
