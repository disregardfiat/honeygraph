  ...
  handleCheckpoint(ws, msg) {
    ...
    if (activeFork) {
      ...
      if (oldFork) {
        ...
        // IMPORTANT: Clear operations from the old fork to prevent memory accumulation
        // Operations have already been processed via ReplicationQueue
        console.log(`[Honeygraph] Clearing ${oldFork.operations.length} operations from previous fork ${activeFork}`);
        oldFork.operations = [];
      }
    }
