...
export class ReplicationQueue {
  constructor({ dgraphClient, forkManager, zfsCheckpoints, logger }) {
    this.dgraph = dgraphClient;
    this.forkManager = forkManager;
    this.zfsCheckpoints = zfsCheckpoints;
    this.logger = logger;
    this.dataTransformer = createDataTransformer(dgraphClient);
  ...
  setupProcessors() {
    ...
    this.queue.process('process-operation', async (job) => {
      ...
      try {
        ...
        const transformedOp = await this.dataTransformer.transformOperation(operation);
        
        // Write to Dgraph
        await this.dgraph.writeOperation(transformedOp);
