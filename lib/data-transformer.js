...
export class DataTransformer {
      ...
      for (const [index, cid] of fileNames.entries()) {
        ...
        if (!((fileFlags || 0) & 2)) {
          ...
          if (fileMutation) {
            ...
            fileMutation.parentPath = { uid: directoryPath.uid };
            ...
            const fileExists = directoryPath.files.some(f => f.uid === fileMutation.uid);
            if (!fileExists) {
              directoryPath.files.push({ uid: fileMutation.uid });
              
              // CRITICAL: Also add to path accumulator for cross-batch accumulation
              if (pathAccumulator.batchMode) {
                pathAccumulator.addFileToPath(fileOwner, dirPath, fileMutation.uid);
              }
              
              logger.debug(`Added file to path`, {
                pathUid: directoryPath.uid,
                pathFullPath: directoryPath.fullPath,
                fileUid: fileMutation.uid,
  ...
  parseMetadataString(metadataString, cids) {
    ...
    // The first part IS the folder name, then file parts follow
    if (!folderString && parts.length > 1) {
      // First part might be folder name if it's not a file part pattern
      const firstPart = parts[0];
      
      // Check if this looks like a folder name (not containing typical file extensions)
      if (firstPart && !firstPart.includes('.') && !firstPart.includes('-')) {
        // Add this as a custom folder with index '1'
        const folderInfo = {
          index: '1',
          name: firstPart,
          parent: '',
        ...
        folders.push(folderInfo);
        folderMap.set('1', folderInfo);
        
        // Now parse files starting from index 1
        const partsPerFile = 4;
        for (let i = 0; i < sortedCids.length; i++) {
          const cid = sortedCids[i];
          const baseIndex = i * partsPerFile + 1;
          
          if (baseIndex < parts.length) {
            // For simple format, parts might be: folder,name,ext,thumb,flags
            // So for file 0: name at index 1, ext at index 2, etc.
            const name = parts[baseIndex] || cid;
            const ext = parts[baseIndex + 1] || '';
            ...
            const metadata = {
              name,
              ext,
              pathIndex: '1', // Use the folder we just created
              thumb,
              flags,
              license,
              labels,
              folder: folderInfo.fullPath,
              fullPath: `${folderInfo.fullPath}/${name}${ext ? '.' + ext : ''}`
            };
