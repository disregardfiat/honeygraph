...
export class DataTransformer {
  ...
  async updatePathWithFile(username, filePath, file, contract, mutations) {
    ...
    // Only update if this is a new path or if the contract is newer
    if (path.uid && path.uid.startsWith('0x')) {
      // This is an existing path from the database
      // Create an update mutation if this contract is newer
      if (!path.newestBlockNumber || contract.blockNumber > path.newestBlockNumber) {
        // Create update mutation for existing path
        const updateMutation = {
          uid: path.uid,
          currentFile: { uid: file.uid },
          newestBlockNumber: contract.blockNumber
        };
        
        // Add to other mutations to update the existing path
        mutations.other.push(updateMutation);
      ...
      // This is a new path, update normally
      if (!path.newestBlockNumber || contract.blockNumber > path.newestBlockNumber) {
        path.currentFile = { uid: file.uid };
        path.newestBlockNumber = contract.blockNumber;
      }
