...
export class DataTransformer {
  ...
  async transformContract(path, contract, mutations) {
    ...
    if (contract.df) {
      ...
      for (const [index, cid] of fileNames.entries()) {
        ...
        if (!((fileFlags || 0) & 2)) {
          // Only create directory paths - files should be contained within directories
          ...
          if (fileMutation) {
            ...
            // Add file to directory's files array
  ...
  buildMutations(mutations, blockInfo) {
    ...
    for (const account of mutations.accounts.values()) {
      // Add new accounts and existing accounts that have been updated in this batch
      if (!account.isExisting || account.hasUpdates) {
        // Clean up metadata fields before adding to mutations
    ...
    if (mutations.paths && mutations.paths.size > 0) {
      ...
      for (const path of mutations.paths.values()) {
        // Debug log to check file associations
        if (path.files && path.files.length > 0) {
          logger.debug('Path has files before mutation', {
            fullPath: path.fullPath,
            fileCount: path.files.length,
            files: path.files.slice(0, 3).map(f => f.uid)
          });
        }
  ...
  async getOrCreatePath(username, fullPath, type = 'directory', mutations) {
    ...
    // Check if path already exists in memory cache
    ...
    try {
      ...
      if (result.path && result.path.length > 0) {
        ...
        userPathMap.set(fullPath, existingPath);
        
        // Also add to mutations map for reference
        if (!mutations.paths) {
          mutations.paths = new Map();
        }
        mutations.paths.set(`${username}:${fullPath}`, existingPath);
