...
export class DataTransformer {
  ...
  async transformOperationInternal(op, blockInfo, mutations) {
    ...
    switch (path[0]) {
      ...
      case 'contract':
        logger.debug('Processing contract operation', { path, dataKeys: Object.keys(data).slice(0, 5) });
  ...
  async ensureAccount(username, mutations) {
    if (!mutations.accounts.has(username)) {
      ...
      if (existingUid) {
        // Use existing account UID - don't create new mutation
  ...
  async transformAuthority(account, publicKey, mutations) {
    await this.ensureAccount(account, mutations);
  ...
  async transformContract(path, contract, mutations) {
    ...
    logger.info('Setting contract mutation', { contractId, hasContract: !!dgraphContract });
    mutations.contracts.set(contractId, dgraphContract);
    logger.info('Contract mutation set', { contractId, mutationsSize: mutations.contracts.size });
    } catch (error) {
  ...
  async updatePathWithFile(username, filePath, file, contract, mutations) {
    const path = await this.getOrCreatePath(username, filePath, 'directory', mutations);
