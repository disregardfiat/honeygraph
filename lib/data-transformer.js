...
export class DataTransformer {
  ...
  async transformContract(path, contract, mutations) {
    ...
    let contractUid = `_:contract_${contractId.replace(/[:/\-]/g, '_')}`;
    try {
      const existingQuery = `
        query getExistingContract($contractId: string) {
          contract(func: eq(id, $contractId)) @filter(type(StorageContract)) {
            uid
          }
        }
      `;
      const existingResult = await this.dgraph.query(existingQuery, { $contractId: contractId });
      if (existingResult.contract && existingResult.contract.length > 0) {
        contractUid = existingResult.contract[0].uid;
        logger.info('Found existing contract, will update', { contractId, uid: contractUid });
      }
    } catch (error) {
      logger.warn('Failed to check for existing contract', { contractId, error: error.message });
    }
    ...
    // If storage node map is present (n object), persist assignments so clients can query by node-change recency
    if (contract.n && typeof contract.n === 'object') {
      dgraphContract.storageNodes = Object.entries(contract.n).map(([nodeKey, username]) => ({
        id: `${contractId}:${nodeKey}`,
        nodeNumber: String(nodeKey),
    ...
    if (contract.df) {
      ...
      // Ensure contract file set is replaced (not appended)
      // Fetch existing files for this contract to detect removals
      const existingFilesForReplace = await this.getExistingContractFiles(contractId);
      const newCidSet = new Set(fileNames);
      const toRemove = existingFilesForReplace.filter(f => !newCidSet.has(f.cid));
      if (toRemove.length > 0) {
        await this.removeFilesFromPaths(toRemove, ownerUsername, mutations);
      }
      ...
      for (const [index, cid] of fileNames.entries()) {
        ...
        if (!((fileFlags || 0) & 2)) {
          if (file) {
            ...
            // Note: We don't maintain a files array on the path
            // Files are linked via their parentPath edge
            // Just increment the count if this is a new file
            
            // Use path accumulator to track files across contracts
            let isNewFile = true;
            if (pathAccumulator.batchMode) {
              // Check if this file was already added in a previous contract
              const accumulatedFiles = pathAccumulator.getPathFiles(fileOwner, dirPath);
              isNewFile = !accumulatedFiles.find(f => f.uid === file.uid);
            } else {
              // Single contract mode - use local tracking
              if (!directoryPath.hasOwnProperty('_fileUids')) {
                directoryPath._fileUids = new Set();
              }
              isNewFile = !directoryPath._fileUids.has(file.uid);
              if (isNewFile) {
                directoryPath._fileUids.add(file.uid);
              }
            }
            
            if (isNewFile) {
              directoryPath.itemCount = (directoryPath.itemCount || 0) + 1;
              
              // CRITICAL: Also add to path accumulator for cross-batch accumulation
              if (pathAccumulator.batchMode) {
                pathAccumulator.addFileToPath(fileOwner, dirPath, {
                  uid: file.uid,
                  name: file.name,
                  ext: file.extension,
                  size: file.size,
              ...
              logger.debug(`Added file to path`, {
                ...
                pathFullPath: directoryPath.fullPath,
                fileUid: file.uid,
                totalFiles: directoryPath.itemCount
              });
            }
            
            logger.debug(`File linked to directory`, {
    ...
    if (contract.n) {
      ...
      // contract.n is like {"1": "disregardfiat", "2": "dlux-io"}
      const storageNodeEntries = Object.entries(contract.n)
        .sort(([a], [b]) => parseInt(a) - parseInt(b))
        .map(([_, nodeAccount]) => nodeAccount);
      
      dgraphContract.nodeTotal = storageNodeEntries.length;
      dgraphContract.isUnderstored = dgraphContract.nodeTotal < dgraphContract.power;
      
      dgraphContract.storageNodes = [];
      for (const nodeAccount of storageNodeEntries) {
      ...
      dgraphContract.nodeTotal = 0;
      dgraphContract.isUnderstored = dgraphContract.power > 0;
      dgraphContract.storageNodes = [];
    }
  ...
  async buildMutations(mutations, blockInfo) {
    ...
    if (pathMutations.size > 0) {
      ...
      for (const [key, path] of pathMutations) {
        ...
        // For paths, we don't need to store the files array
        // Files are linked via their parentPath edge, and dgraph will find them with ~parentPath
        const pathToSave = { ...path };
        delete pathToSave.files; // Remove files array - dgraph uses reverse edges
        
        // For existing paths, only update if there are changes
        if (path.uid && path.uid.startsWith('0x')) {
          // Only update itemCount for existing paths
          const pathUpdate = {
            uid: path.uid,
            'dgraph.type': 'Path',
            itemCount: path.itemCount || 0
          };
          dgraphMutations.push(pathUpdate);
        } else {
          // For new paths, add the complete path (without files array)
          dgraphMutations.push(this.validateFieldTypes(pathToSave));
        }
      }
    }
  ...
  parseMetadataString(metadataString, cids) {
    ...
    for (const [index, name] of Object.entries(presetFolders)) {
      folderMap.set(index, {
        ...
        name,
        parent: '',
        fullPath: '/' + name  // Ensure preset folders have leading slash
      });
    }
  ...
  async getOrCreatePath(username, fullPath, type = 'directory', mutations) {
    logger.info('getOrCreatePath called', { username, fullPath, type });
    ...
    if (mutations.paths && mutations.paths.has(mutationKey)) {
      logger.info('Found path in current mutations', { username, fullPath, mutationKey });
    ...
    // Then check memory cache - but only trust it if it has a real UID
    if (userPathMap.has(fullPath)) {
      const cachedPath = userPathMap.get(fullPath);
      
      // Only use cached path if it has been committed (has a real UID)
      if (cachedPath.uid && cachedPath.uid.startsWith('0x')) {
        logger.info('Found committed path in memory cache', { username, fullPath, uid: cachedPath.uid });
        ...
        logger.info('Ignoring uncommitted path in memory cache', { 
          username, 
          fullPath, 
          uid: cachedPath.uid 
        });
        // Clear it from cache since it's not committed
        userPathMap.delete(fullPath);
      }
    }
    
    // Check if path already exists in database
    // Filter paths by owner username without cascade at root level
    const existingPathQuery = `
      query getExistingPath($username: string, $fullPath: string) {
        path(func: type(Path)) @filter(eq(fullPath, $fullPath) AND has(owner)) {
          uid
          fullPath
          pathName
          pathType
    ...
    try {
      ...
      logger.debug('Path query result', {
        username,
        fullPath,
        foundPaths: result.path?.length || 0,
        paths: result.path?.map(p => ({
          uid: p.uid,
      ...
      if (result.path && result.path.length > 0) {
        // Filter out paths that don't have the matching owner
        const pathsWithMatchingOwner = result.path.filter(p => p.owner && p.owner.username === username);
        
        if (pathsWithMatchingOwner.length === 0) {
          logger.debug('No paths found with matching owner', { username, fullPath });
          // Fall through to create new path
        } else {
          // If multiple paths exist, use the one with the most files
          let existingPath = pathsWithMatchingOwner[0];
            if (pathsWithMatchingOwner.length > 1) {
              logger.warn('Multiple paths found for same owner+fullPath', {
                username,
                fullPath,
                count: pathsWithMatchingOwner.length,
                paths: pathsWithMatchingOwner.map(p => ({ uid: p.uid, fileCount: p.files?.length || 0 }))
              });
              
              // Pick the one with the most files
              existingPath = pathsWithMatchingOwner.reduce((best, current) => {
        ...
        logger.debug('Found existing path in database', { 
          ...
          existingItemCount: existingPath.itemCount,
        ...
        const mutablePath = {
          ...
          itemCount: existingPath.itemCount || 0, // Preserve existing count - will be incremented as files are added
          // Note: files are not stored here - they're linked via parentPath edge
        ...
        logger.debug('Created mutable path with combined files', {
          ...
          totalFiles: combinedFileMap.size
        });
        ...
        return mutablePath;
        } // Close the else block for pathsWithMatchingOwner.length > 0
      }
    } catch (error) {
    ...
    // For new paths, we'll use a unique external ID that can be used for upserts
    const externalId = `${username}:${fullPath}`;
    const pathId = `path_${username}_${fullPath}`.replace(/[^a-zA-Z0-9]/g, '_');
    const pathEntity = {
      uid: `_:${pathId}`,
      'dgraph.type': 'Path',
      'Path.externalId': externalId,  // Add external ID for upsert deduplication
      fullPath,
  ...
  calculatePathCounts(mutations) {
    // Since we're now maintaining itemCount as files are added,
    // this function only needs to add subdirectory counts
    ...
    // Add subdirectory counts to each directory
    for (const path of sortedPaths) {
      if (path.pathType === 'directory') {
        ...
        // Add subdirectory count to existing file count
        path.itemCount = (path.itemCount || 0) + directoryCount;
        
        logger.debug('Path count calculated', {
          fullPath: path.fullPath,
          fileCount: path.itemCount - directoryCount,
          directoryCount,
          totalItems: path.itemCount
        });
      }
    }
  }
