...
export class DgraphClient {
  ...
  async setSchema(schema, dropAll = false) {
    const maxRetries = 10;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        if (dropAll) {
          // Drop all data and schema first
          const dropOp = new dgraph.Operation();
          dropOp.setDropAll(true);
          await this.client.alter(dropOp);
          this.logger.info('Dropped all data and schema', { 
            namespace: this.namespace || 'default' 
          });
        }
        
        // Debug: Check for duplicate predicates in schema
        ...
        lines.forEach((line, index) => {
          ...
          if (match) {
            ...
            if (predicates.has(predicate)) {
              this.logger.error(`DUPLICATE PREDICATE in schema: "${predicate}"`, {
                firstLine: predicates.get(predicate) + 1,
                duplicateLine: index + 1,
                namespace: this.namespace
              });
              hasDuplicates = true;
            } else {
              predicates.set(predicate, index);
            }
          }
        });
        
        if (hasDuplicates) {
          throw new Error('Schema contains duplicate predicate definitions');
        }
        ...
        if (error.message.includes('server is not ready') && attempt < maxRetries) {
          this.logger.info(`Dgraph not ready, retrying in ${retryDelay/1000}s... (attempt ${attempt}/${maxRetries})`, {
            namespace: this.namespace
          });
