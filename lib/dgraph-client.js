...
export class DgraphClient {
  ...
  connect() {
    try {
      ...
      this.connected = true;
      this.logger.info('Connected to Dgraph', { url: this.url });
    } catch (error) {
  ...
  async initializeSchema() {
    ...
    try {
      ...
      this.logger.error('Failed to initialize schema', { error: error.message });
      throw error;
    }
  }
  ...
  async writeOperation(operation) {
    ...
    try {
      ...
      if (Array.isArray(operation)) {
        ...
        mutation.setSetJson(operation);
        await txn.mutate(mutation);
      } else {
        // Single operation
        const mutation = this.createOperationMutation(operation);
        await txn.mutate(mutation);
      }
      await txn.commit();
      return { success: true };
