...
class WSForkHandler extends EventEmitter {
  ...
  async handleAuthResponse(ws, msg) {
    try {
      ...
      const dhive = await import('@hiveio/dhive'); // For API calls
      ...
      // Verify signature using hive-tx
      let isValid = false;
      ...
      try {
        ...
        const pubKey = hiveTx.PublicKey.from(publicKey);
    ...
    if (this.wss && this.wss.clients) {
      this.wss.clients.forEach(ws => {
        if (ws.readyState === 1) { // 1 = OPEN
          ...
          const secondsSinceLastMessage = ws.lastMessage 
            ? Math.floor((Date.now() - ws.lastMessage) / 1000)
            : null;
          
          // Get fork details if available
          const activeForkHash = this.activeForks.get(ws.nodeId);
          const activeFork = activeForkHash ? this.forks.get(activeForkHash) : null;
          ...
          nodes.push({
            ...
            activeFork: activeForkHash || null,
            authError: ws.lastAuthError || null,
            // Extended metadata
            prefix: ws.prefix || null,
            token: ws.token || null,
            source: ws.source || null,
            version: ws.version || null,
            lastMessage: ws.lastMessage ? new Date(ws.lastMessage).toISOString() : null,
            secondsSinceLastMessage: secondsSinceLastMessage,
            // Fork details
            forkRoot: activeFork ? activeFork.prevHash : null,
            forkBlock: activeFork ? activeFork.blockNum : null,
            forkNodeCount: activeFork ? activeFork.nodes.size : null
          });
        }
      });
    }
