...
class WSForkHandler extends EventEmitter {
  ...
  handleMessage(ws, msg) {
    ...
    if (!msg || typeof msg !== 'object') {
      console.warn(`Invalid message from ${ws.nodeId}:`, msg);
      ws.send(JSON.stringify({
        type: 'error',
        error: 'Invalid message format'
      }));
    ...
    try {
      ...
      if (!account || !signature || !message) {
        const error = {
          ...
          details: 'Please provide account, signature, and message fields in your auth_response'
        };
        ws.lastAuthError = error;
        ws.send(JSON.stringify({
          type: 'auth_failed',
          ...error
        }));
      ...
      if (!messageData.challenge || 
        ...
        const error = {
          ...
          details: 'The challenge in your signed message does not match the one sent by the server'
        };
        ws.lastAuthError = error;
        ws.send(JSON.stringify({
          type: 'auth_failed',
      ...
      const authorizedNodes = (process.env.AUTHORIZED_HONEYCOMB_NODES || '')
        .split(',')
        .filter(n => n.trim())
      ...
      if (authorizedNodes.length > 0 && !authorizedNodes.includes(account.toLowerCase())) {
        const error = {
          ...
          details: `Account '${account}' is not in the authorized nodes list. Please contact the honeygraph administrator to be added. Authorized accounts: ${authorizedNodes.join(', ')}`
        };
        ws.lastAuthError = error;
        ws.send(JSON.stringify({
          type: 'auth_failed',
      ...
      if (!accounts || accounts.length === 0) {
        const error = {
          reason: 'Account not found on Hive',
          details: `The account '${account}' does not exist on the Hive blockchain. Please check the account name.`
        };
        ws.lastAuthError = error;
        ws.send(JSON.stringify({
          type: 'auth_failed',
      ...
      try {
        ...
        console.log('Signature format:', {
          ...
          sample: signature.substring(0, 20) + '...',
          isHex: /^[0-9a-fA-F]+$/.test(signature)
        });
        
        // Try to parse the signature based on its format
        let sig;
        
        // Check if it's a Hive-style compact signature (starts with "1f" or "20")
        if (signature.startsWith('1f') || signature.startsWith('20')) {
          // This is likely the compact format from Hive
          sig = dhive.Signature.fromString(signature);
        } else if (signature.length === 130 && /^[0-9a-fA-F]+$/.test(signature)) {
          // Hex signature - convert to Signature object
          const sigBuffer = Buffer.from(signature, 'hex');
          
          // The buffer should be 65 bytes for a valid signature
          if (sigBuffer.length === 65) {
            // Recovery parameter is the first byte
            const recovery = sigBuffer[0];
            const r = sigBuffer.slice(1, 33);
            const s = sigBuffer.slice(33, 65);
            
            // Create signature with proper recovery parameter adjustment
            sig = new dhive.Signature(r, s, recovery - 31);
          } else {
            throw new Error(`Invalid hex signature length: ${sigBuffer.length}`);
          }
        } else {
        ...
        isValid = pubKey.verify(messageHash, sig);
      ...
      if (!isValid) {
        const error = {
          ...
          details: `The signature could not be verified against the active key for account '${account}'. Please ensure you are signing with your Hive active private key (not posting key).`
        };
        ws.lastAuthError = error;
        ws.send(JSON.stringify({
          type: 'auth_failed',
      ...
      ws.lastAuthError = null; // Clear any previous auth errors
      ...
      const authError = {
        ...
        details: error.message || 'An unexpected error occurred during authentication. Please check your credentials and try again.'
      };
      ws.lastAuthError = authError;
      ws.send(JSON.stringify({
        type: 'auth_failed',
        ...authError
      }));
  ...
  /**
   * Get list of connected nodes with their account names
   */
  getConnectedNodes() {
    const nodes = [];
    
    // Iterate through all WebSocket clients
    if (this.wss && this.wss.clients) {
      this.wss.clients.forEach(ws => {
        if (ws.readyState === 1) { // 1 = OPEN
          // Generate a safe node identifier without exposing IP
          const safeNodeId = ws.hiveAccount || `node-${ws.nodeId ? ws.nodeId.split('-').pop() : 'unknown'}`;
          
          nodes.push({
            nodeId: safeNodeId,
            account: ws.hiveAccount || 'pending-auth',
            authenticated: ws.authenticated || false,
            connectedAt: ws.connectedAt || new Date(parseInt(ws.nodeId?.split('-').pop() || Date.now())).toISOString(),
            activeFork: this.activeForks.get(ws.nodeId) || null,
            authError: ws.lastAuthError || null
          });
        }
      });
    }
